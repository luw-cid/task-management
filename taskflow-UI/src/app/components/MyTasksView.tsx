    ...
    <div className="flex flex-col gap-6 px-8 py-8 w-full max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Quản lý và theo dõi tất cả các công việc được giao cho bạn trên hệ thống
          </p>
        </div>

        <button
          onClick={onCreateTask}
          className="flex items-center gap-2 rounded-xl bg-primary px-4 py-2.5 text-sm font-semibold text-white hover:bg-primary/90 transition-all shadow-md shadow-primary/25 flex-shrink-0"
        >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        >
          ...
          <div>
            <p className="text-2xl font-bold text-foreground leading-none">{metrics.total}</p>
            <p className="text-xs font-medium text-muted-foreground mt-1">Tất cả task</p>
          </div>
        </div>
        ...
        >
          ...
          <div>
            <p className="text-2xl font-bold text-amber-400 leading-none">{metrics.active}</p>
            <p className="text-xs font-medium text-muted-foreground mt-1">Còn hạn</p>
          </div>
        </div>
        ...
        >
          ...
          <div>
            <p className="text-2xl font-bold text-rose-400 leading-none">{metrics.overdue}</p>
            <p className="text-xs font-medium text-muted-foreground mt-1">Quá hạn</p>
          </div>
        </div>
        ...
        >
          ...
          <div>
            <p className="text-2xl font-bold text-emerald-400 leading-none">{metrics.completed}</p>
            <p className="text-xs font-medium text-muted-foreground mt-1">Đã hoàn thành</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col rounded-2xl border border-border bg-card overflow-hidden shadow-sm">
        {/* Navigation Tabs Bar */}
        <div className="flex flex-wrap items-center justify-between gap-4 px-6 pt-5 pb-4 border-b border-border/80 bg-secondary/10">
          {/* Tabs */}
          <div className="flex items-center gap-1.5 p-1 rounded-xl bg-secondary/40 border border-border/60">
            {tabsConfig.map((tab) => {
              const Icon = tab.icon;
              ...
              return (
                ...
                >
                  <Icon className={`h-4 w-4 ${isActive ? tab.colorClass : "text-muted-foreground"}`} />
                  <span>{tab.label}</span>
                  <span
...
function MyTaskRow({ task, onOpen }: { task: DashboardTask; onOpen: (task: DashboardTask) => void }) {
  ...
  return (
    <motion.div
      ...
      onClick={() => onOpen(task)}
      className={`group grid grid-cols-1 md:grid-cols-[1fr_120px_140px_130px_100px_40px] items-center gap-4 px-6 py-3.5 hover:bg-secondary/30 transition-colors cursor-pointer ${
    ...
    >
      ...
      <div className="flex items-center justify-center">
        <span
          className={`inline-flex items-center justify-center rounded-md px-2.5 py-1 text-xs font-semibold border ${p.bg}`}
        >
          {p.label}
        </span>
      </div>
      ...
      <div className="flex items-center justify-center">
        <div
          className={`flex items-center gap-1.5 text-xs font-medium ${
            isOverdue ? "text-rose-400" : done ? "text-muted-foreground" : "text-foreground/80"
          }`}
        >
      ...
      {/* Status Badge (Quá hạn vs Còn hạn vs Hoàn thành) */}
      <div className="flex items-center justify-center">
        {done ? (
